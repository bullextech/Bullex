...
type VerifyState =
  | { status: "loading" }
  | { status: "valid"; username: string; name: string; expiresAt: string }
  | { status: "invalid"; message: string };

export default function TeamReset() {
  const [, params] = useRoute("/team-reset/:token");
  ...
  useEffect(() => {
    ...
    (async () => {
      if (!token) {
        setVerify({ status: "invalid", message: "Missing reset token." });
        return;
      }
      try {
        const r = await fetch(`/api/team/reset/${encodeURIComponent(token)}`);
        const j = await r.json();
        if (cancelled) return;
        if (r.ok && j.valid) {
          setVerify({ status: "valid", username: j.username, name: j.name, expiresAt: j.expiresAt });
        } else {
          setVerify({ status: "invalid", message: j.message || "This reset link is no longer valid." });
        }
      } catch (e: any) {
        if (!cancelled) setVerify({ status: "invalid", message: e?.message || "Could not verify the reset link." });
      }
    })();
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const j = await r.json();
      if (!r.ok) throw new Error(j.message || "Could not reset password.");
      setDone(true);
    } catch (e: any) {
      setSubmitError(e?.message || "Could not reset password.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      <header className="border-b border-border bg-background px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground uppercase tracking-[0.2em]">Password Reset</p>
          </div>
        </div>
        <ThemeToggle />
      </header>
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-md border border-border bg-card rounded-md shadow-sm overflow-hidden" data-testid="card-team-reset">
          <div className="border-b border-border bg-muted/30 px-5 py-4 flex items-center gap-2.5">
            <KeyRound className="w-4 h-4 text-primary" />
            <div>
              <h2 className="text-sm font-bold tracking-tight">Set a New Password</h2>
              <p className="text-[10px] text-muted-foreground">One-time secure reset</p>
            </div>
          </div>
          ...
          <div className="p-5">
            ...
            {verify.status === "invalid" && (
              <div className="text-center py-6" data-testid="text-reset-invalid">
                ...
                <p className="text-[10px] text-muted-foreground mt-3">
                  Please ask your administrator to issue a new reset link.
                </p>
              </div>
            )}

            {verify.status === "valid" && done && (
              <div className="text-center py-6" data-testid="text-reset-success">
                <CheckCircle2 className="w-10 h-10 text-emerald-600 mx-auto mb-2" />
                <p className="text-sm font-semibold mb-1">Password updated</p>
                <p className="text-xs text-muted-foreground">You can now sign in to Bullex with your new password.</p>
                <Button
                  className="mt-4 rounded-none text-xs font-bold uppercase tracking-wider h-9"
                  onClick={() => navigate("/")}
                  data-testid="btn-reset-go-login"
                >
                  Continue to sign in
                </Button>
              </div>
            ...
            {verify.status === "valid" && !done && (
              <form onSubmit={onSubmit} className="space-y-4">
                <div className="text-xs text-muted-foreground border border-border bg-muted/30 rounded-md px-3 py-2">
                  <p>Hello <strong className="text-foreground">{verify.name}</strong></p>
                ...
                {submitError && (
                  <p className="text-xs text-destructive border border-destructive/20 bg-destructive/5 p-2" data-testid="text-reset-error">
                    {submitError}
                  </p>
                )}
                <Button
                ...
                >
                  {submitting ? <><Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" /> Updating...</> : "Set New Password"}
                </Button>
              </form>
            )}
          </div>
