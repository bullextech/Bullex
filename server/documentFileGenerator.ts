...
function buildFooterParagraphs(submittedBy?: string): Paragraph[] {
  const out: Paragraph[] = [
...
export async function generateDocx(docId: string, title: string, content: string, submittedBy?: string): Promise<string> {
  ...
  children.push(...buildFooterParagraphs(submittedBy));
...
export async function generatePdf(docId: string, title: string, content: string, submittedBy?: string): Promise<string> {
...
function addPdfFooter(doc: PDFKit.PDFDocument, leftMargin: number, pageWidth: number, submittedBy?: string) {
