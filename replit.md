...
- `/trading-terminal` - Bullex Trading Terminal: full-screen Bloomberg-style order-book / matching terminal built from the supplied HTML mockup (`attached_assets/bullex-trading-terminal_*.html`), recolored from the source gold/dark theme to the Bullex theme (brand-mark + execute/"Back to Bullex" button burgundy #990000; brand wordmark, active tabs, focus borders, price/value highlights blue #0084be; **green=buy / red=sell retained as functional trading colours**). Layout: topbar (Order Book / Matched Trades / My Enquiries tabs + live clock + user pill) + scrolling commodity ticker + 3-column grid (Post Enquiry form | order book table with per-row compatibility scoring vs the selected enquiry + MATCH buttons | Match Detail panel with top-5 candidate matches), plus a match-confirm modal (proposed trade terms + compatibility breakdown) and toasts. Demo data uses Bullex `commodityCategories` (Iron Ore, Bauxite, Manganese Ore, Copper Cathode, Copper Concentrate, Aluminium Ingots, Gasoil 10ppm/50ppm, LHC, HSFO, HSGO, Petcoke Anode/Fuel, NPK) and incoterms CIF/FOB/CFR/DAP. Served statically from `client/public/trading-terminal.html`, embedded via a sandboxed iframe (`sandbox="allow-scripts allow-popups allow-forms"`, no allow-same-origin) in `client/src/pages/trading-terminal.tsx` with a burgundy "Back to Bullex" link (→ `/trading`).
  - **Live data wiring**: `client/src/components/trading-terminal-frame.tsx` is the React bridge (mirrors `trade-bank-frame.tsx`). When authenticated (admin/team/client) it fetches `/api/trade-enquiries`, maps each enquiry to the terminal shape (`id`=enquiryRef, `side`, `commodity`=product, `volume`=quantity, `price`, `origin`=origin||loadingPort, `destination`=dischargePort, `grade` inferred from specifications, `incoterms`, `payment`=paymentTerms, `participant`=(sell→sellerName/producer/createdBy; buy→buyerName/createdBy), `status`=linkedTradeRef?"matched":"open", `ts`=createdAt, `mine`=participant matches logged-in company), filters out cancelled/closed/rejected/withdrawn, and pushes `{enquiries, user}` into the iframe via `postMessage({source:"bullex-parent", payload})`. The iframe-side script listens for that message, validates `source`, `esc()`-escapes all values, replaces the seeded demo book with live enquiries (`liveMode=true`), re-renders, and disables the demo price-tick simulation. Handshake: iframe posts `{source:"bullex-iframe",ready:true}` on load; parent posts only after the handshake (or iframe `onLoad`). Security boundary preserved (no allow-same-origin); logged-out visitors get no postMessage and see the static demo book + "Demo Trader" avatar.
  - **Scope**: order book + My Enquiries are LIVE read-only for authenticated users (mirrors the trade-bank non-submitting precedent). Post Enquiry and Execute Match remain terminal-local/demo (non-persisting) — candidate for backend wiring as a follow-up.
  - **Discovery**: linked from the public top navbar as "Trading Terminal" (logged-out) and via an "Open Trading Terminal" button on the Deal Desk header (`/trading`).
