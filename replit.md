...
- **Document Generator**: Generate Deal Recap, FCO, SCO, ICPO, SPA, LOI, POP, POF, BCL, NCNDA, BL, COO, TFR with DOCX/PDF output, auto-email with PDF attachment, KYC client auto-fill, Review step (preview content before generating DOCX), Digital Signatures (draw-to-sign buyer/admin signature for all doc types, embedded in regenerated PDF/DOCX), Convert to PDF available for all doc types after signing
  - **TFR template**: Transaction Feasibility Report — internal feasibility assessment with 11 numbered sections (Executive Summary, Parties, Product Feasibility, Market Analysis, Financial Feasibility, Trade Finance Feasibility, Logistics Feasibility, Legal & Compliance Review, Risk Assessment, Security Structure, Overall Feasibility Conclusion) plus Prepared/Approved By signature block. Form is a transient key/value map (`productDetails.tfrData`) — not schema columns; each section is a form accordion with text inputs and single-choice Selects for conclusions/risk ratings/recommendation. Exec Summary overview auto-fills from linked trade (product, origin, destination, seller, buyer, quantity, contract value, Incoterms). DOCX/PDF use dedicated builders (`buildTfrDocx`/`buildTfrPdf`, gated by `isTfrContent`) that infer structure from the natural content string: numbered lines → shaded section headings, lines ending ":" → bold sub-labels, `Label: value` → borderless 2-col key/value tables, `a | b` rows → real tables with shaded header (financial economics "Particulars | Amount (USD)", risk rating "Risk Category | Rating"), and `[X]`/`[ ]` checkbox lines for conclusions/recommendation.
    - **TFR workflow (after Deal Recap)**: The TFR is prepared by the admin/risk team after a Deal Recap. Each Deal Recap row in the document list has a "Prepare TFR" button that opens the TFR form pre-filled from that recap. The TFR form also has a "Source Deal Recap" selector. Both call `fillTfrFromRecap`, which parses the Deal Recap content table (` Label │ Value` rows) and copies available fields into `tfrData`: Contract Reference→reference, Seller→seller, Buyer→buyer, Commodity→product+commodity, Country of Origin→origin+loadingPort, Quality/Specification→specifications, Delivery Basis→deliveryTerms, Contractual Quantity→quantity, Contract Price & Currency→contractValue, Payment Terms→paymentInstrument, and the Shipping Terms "Port of Discharge (POD):" line→destination+dischargePort. Placeholder values (underscores/dashes) are skipped.
